...
function updateBorders(color) {
    ...
    if(color) {
		...
		var nCol		= datagrid1.getCol() > 0 ? datagrid1.getCol() : 1
		var nRow	= datagrid1.getRow() > 0 ? datagrid1.getRow() : datagrid1.getFixedRows();
		
		hexColor+="!important";
		var flg_ = $('#style_flg').val();
		//console.log(flg_);
		if(flg_ == '1'){
			datagrid1.setColStyle(nCol, 'data', 'background-color', hexColor);
		}else if(flg_ == '2'){
			datagrid1.setRowStyle(nRow, 'data', 'background-color', hexColor);
		}else{
			datagrid1.setCellStyle('background-color', nRow, nCol, nRow, nCol, hexColor);
		}
    }
